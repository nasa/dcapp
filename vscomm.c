...
int vscomm_activate(char *host, int port, char *rate_spec, char *default_rate)
{
    ...
    else
    {
        ...
        sim_write("trick.var_clear()\n");
    }

    sim_write("trick.var_pause()\n");
...
//    sim_write("trick.var_debug(1)\n"); // FOR DEBUGGING
    sim_write("trick.var_unpause()\n");
