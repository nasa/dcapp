...
static void render_primitives(struct node *list)
{
    ...
    for (current = list; current != NULL; current = current->p_next)
    {
        ...
        {
            case Panel:
                setup_panel(current->object.panel.orthoX, current->object.panel.orthoY, -1, 1, current->object.panel.color.R, current->object.panel.color.G, current->object.panel.color.B);
                break;
            case Condition:
            ...
            case Container:
                ...
                break;
            case Line:
                line_start(current->object.line.linewidth, current->object.line.color.R, current->object.line.color.G, current->object.line.color.B);
            ...
            case Polygon:
                ...
                {
                    polygon_fill_start(current->object.poly.FillColor.R, current->object.poly.FillColor.G, current->object.poly.FillColor.B);
                ...
                {
                    polygon_outline_start(current->object.poly.linewidth, current->object.poly.LineColor.R, current->object.poly.LineColor.G, current->object.poly.LineColor.B);
                    for (sublist = current->object.poly.Vertices; sublist != NULL; sublist = sublist->p_next)
                    {
                        geo = GetGeometry(sublist);
                        gfx_vertex(geo.refx, geo.refy);
            ...
            case Rectangle:
                ...
                if (current->object.rect.fill) rectangle_fill(current->object.rect.FillColor.R, current->object.rect.FillColor.G, current->object.rect.FillColor.B,
                    0, 0, 0, geo.height, geo.width, geo.height, geo.width, 0);
                if (current->object.rect.outline) rectangle_outline(current->object.rect.linewidth, current->object.rect.LineColor.R, current->object.rect.LineColor.G, current->object.rect.LineColor.B,
                    0, 0, 0, geo.height, geo.width, geo.height, geo.width, 0);
                container_end();
                break;
            case Circle:
                ...
                if (current->object.circle.fill) circle_fill(geo.refx, geo.refy, *(current->object.circle.radius), current->object.circle.segments,
                    current->object.circle.FillColor.R, current->object.circle.FillColor.G, current->object.circle.FillColor.B);
                if (current->object.circle.outline) circle_outline(geo.refx, geo.refy, *(current->object.circle.radius), current->object.circle.segments,
                    current->object.circle.LineColor.R, current->object.circle.LineColor.G, current->object.circle.LineColor.B, current->object.circle.linewidth);
                break;
            case String:
