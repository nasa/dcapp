...
pl_app_load(plApiRegistryI* ptApiRegistry, plAppData* ptAppData)
{
    ...
    gptVfs->mount_directory("/shaders-terrain", "../../pl-terrain/shaders", PL_VFS_MOUNT_FLAGS_NONE);
    gptVfs->mount_directory("/assets", "../../pl-terrain/data", PL_VFS_MOUNT_FLAGS_NONE);
    gptVfs->mount_directory("/tiles", "../../pl-terrain/out-temp", PL_VFS_MOUNT_FLAGS_NONE);
    gptVfs->mount_directory("/cache", "../../pl-terrain/cache", PL_VFS_MOUNT_FLAGS_NONE);
    gptVfs->mount_directory("/shaders", "../shaders", PL_VFS_MOUNT_FLAGS_NONE);
    gptVfs->mount_directory("/shader-temp", "../../pl-terrain/shader-temp", PL_VFS_MOUNT_FLAGS_NONE);
    gptFile->create_directory("../../pl-terrain/shader-temp");
    gptFile->create_directory("../../pl-terrain/cache");
    ...
    tPlanetInit.uOutputWidth = 1024;
    tPlanetInit.uOutputHeight = 1024;
...

    {
        ...
        for(uint32_t i = 0; i < 8; i++)
        {
            for(uint32_t j = 0; j < 8; j++)
            {
                ...
                float fLongitude = atanf(fX / fY);
                
                float fR = fX / sinf(fLongitude);
                float fLatitude = PL_PI_2 - 2.0f * atanf(fR / (2.0f * tPlanetInfo.fRadius));

                atTiles[uTileIndex].fLongitude = pl_degreesf(fLongitude);
                atTiles[uTileIndex].fLatitude = pl_degreesf(fLatitude);
        ...
        tPlanetInit.atTextures[0].fLatitude = atTiles[37].fLatitude;
        tPlanetInit.atTextures[0].fLongitude = atTiles[37].fLongitude;
        tPlanetInit.atTextures[0].fMetersPerPixel = 800.0f;
        tPlanetInit.atTextures[0].pcPath = "/assets/hazard.png";
...

    {
        ...
        tPlanetInit.atTextures[0] = {};

        ptAppData->ptPlanet1 = gptPlanet->create_planet(ptCmdBuffer, tPlanetInit, &tPlanetInfo);
    }
...
pl_app_shutdown(plAppData* ptAppData)
{
    ...
    gptPlanet->cleanup_planet(ptAppData->ptPlanet1);
    gptPlanet->cleanup();
    gptResource->cleanup();
...
pl_app_update(plAppData* ptAppData)
{
    ...
    if(gptUI->begin_window("Debug", NULL, 0))
    {
        ...
        plPlanetRuntimeOptions tOptions = gptPlanet->get_runtime_options(ptAppData->ptPlanet0);
        ...
        gptUI->slider_float("Light Direction X", &tOptions.tLightDirection.x, -1.0f, 1.0f, 0);
        gptUI->slider_float("Light Direction Y", &tOptions.tLightDirection.y, -1.0f, 1.0f, 0);
        gptUI->slider_float("Light Direction Z", &tOptions.tLightDirection.z, -1.0f, 1.0f, 0);
        tOptions.tLightDirection = pl_norm_vec3(tOptions.tLightDirection);
        ...
        {
            gptPlanet->reload_shaders(ptAppData->ptPlanet0);
        }

        gptPlanet->set_runtime_options(ptAppData->ptPlanet0, tOptions);
        gptUI->end_window();
    }
    ...
    gptPlanet->prepare(ptAppData->ptPlanet1,  ptCmdBuffer);
    pl_end_cpu_sample(gptProfile, 0);
    gptStarter->submit_temporary_command_buffer(ptCmdBuffer);

    ptCmdBuffer = gptStarter->get_command_buffer();
    pl_begin_cpu_sample(gptProfile, 0, "terrain");
    gptPlanet->render(ptAppData->ptPlanet0, aptCamera[0], ptCmdBuffer);
    gptPlanet->render(ptAppData->ptPlanet1, aptCamera[1], ptCmdBuffer);
    pl_end_cpu_sample(gptProfile, 0);
    ...
    if(ImGui::Begin("View 0"))
    {
        ...
        ImTextureID tTexture = gptDearImGui->get_texture_id_from_bindgroup(gptStarter->get_device(), gptPlanet->get_texture(ptAppData->ptPlanet0));
    ...
    {
        ImVec2 tContextSize = ImGui::GetContentRegionAvail();
        gptCamera->set_aspect(aptCamera[1], tContextSize.x / tContextSize.y);

        ImTextureID tTexture = gptDearImGui->get_texture_id_from_bindgroup(gptStarter->get_device(), gptPlanet->get_texture(ptAppData->ptPlanet1));
        ImGui::Image(tTexture, tContextSize);
    }
    ImGui::End();
