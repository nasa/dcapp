...
void _process_node(xmlNodePtr xml_node, DcAppLookup *lookup) {
    ...
    switch (dc_app_xml_node_to_elem_type(xml_node)) {
        ...
        case DC_APP_ELEM_TYPE_VARIABLE: {
            ...
            if (raw_name) {
                strncpy(clean_name, (char *)raw_name, DC_VALUE_STRING_BUFFER_SIZE - 1);
                xmlFree(raw_name);
            } else {
            ...
            // type
            xmlChar *raw_type = xmlGetProp(xml_node, (xmlChar *)("Type"));
            char     clean_type[DC_VALUE_STRING_BUFFER_SIZE];
            if (raw_type) {
                strncpy(clean_type, (char *)raw_type, DC_VALUE_STRING_BUFFER_SIZE - 1);
                xmlFree(raw_type);
            } else {
            ...
            // xmlChar *raw_init_value = xmlGetProp(xml_node, (xmlChar *)("InitialValue"));
