...
int  init_ent_default_param(
                             /* RETURN: -- Alwaya return zero */
 int which,                  /* IN:     -- Agent type */
 ENT_PARAM_S *epar,          /* INOUT:  -- UDP data structure */
 int ilen,                   /* IN:     -- Input packet length */
 int olen)                   /* IN:     -- Output packet length */
{
  char  *senv;
  int   i;
  struct hostent  *phst;
  ...
  epar->domain  = ENT_NET_DOMAIN;       /* NETWORK domain only for now */
  epar->base_port = ENT_BASE_PORT;

  epar->in_pack_len  = ilen;            /* input packet length */
  epar->out_pack_len = olen;            /* output packet length */

  if (epar->insize_netbuf < epar->in_pack_len+48)
      epar->insize_netbuf = ENT_BUFLEN_BYTES;

  if (epar->outsize_netbuf < epar->out_pack_len+48)
      epar->outsize_netbuf = ENT_BUFLEN_BYTES;
...
int  clear_param( ENT_PARAM_S * epar )
{
  char *par = (char*)epar;
...
int  create_udp_socket( int which, ENT_PARAM_S *epar)
{
  ...
  if ( !strcmp( epar->proto, "udp") || !strcmp( epar->proto, "UDP") ) {
    type = SOCK_DGRAM;
  }
  else {
    /* type = SOCK_STREAM; */
  ...
  /* ADDRESS REUSABLE */
  if ( setsockopt( sd, SOL_SOCKET, SO_REUSEADDR, (char *)&optval, sizeof(optval))
              == -1 ) {
  ...
  if ( setsockopt( sd, SOL_SOCKET, SO_RCVBUF,
       (char *)&epar->insize_netbuf, sizeof(epar->insize_netbuf)) == -1 ) {
  ...
  if ( setsockopt( sd, SOL_SOCKET, SO_SNDBUF,
       (char *)&epar->outsize_netbuf, sizeof(epar->outsize_netbuf)) == -1 ) {
  ...
  else  fprintf(stderr,"ERROR/create_udp_socket: invalid agent type %d\n",which);

  return( sd );

}/*EOF create_udp_socket*/
...
int probe_server_synch( ENT_PARAM_S *epar)
{
  ...
  {
    if ( sendto(epar->socket,(char*)&swapbuf,SYNCH_BYTE_LEN ,0,
            (struct sockaddr*)&epar->yo_sckaddr,len_sckaddr) == -1 ) {
      if ( errno != ECONNREFUSED )
        fprintf(stderr,"ERROR/probe_server_synch: sendto errno %d\n", errno );
    }
    ...
    if (data_ready(epar->socket)) 
    {
      ...
      /* Get ACK/response */
      if ((clen = recvfrom( epar->socket, (char*)&cbuf,SYNCH_BYTE_LEN , 0,
                (struct sockaddr*)&epar->yo_sckaddr, &len_sckaddr)) == SYNCH_BYTE_LEN) 
      {
        if (synch_word == cbuf) 
        {
          /* Send ACK */
         ...
         fprintf(stderr,"ERROR/probe_server_synch: recvfrom errno %d\n", errno);
      }
    }
...
int wait_client_synch( ENT_PARAM_S *epar)
{
  ...
  while ( !dataready)
  {
    ...
    {
      if ( recvfrom(epar->socket, (char*)&cbuf,SYNCH_BYTE_LEN , 0,
                (struct sockaddr*)&epar->yo_sckaddr, &len_sckaddr) == -1) {
  ...
  do {
    if ((dataready= data_ready(epar->socket)) >= SYNCH_BYTE_LEN)
       recvfrom( epar->socket, (char*)&cbuf,SYNCH_BYTE_LEN , 0,
               (struct sockaddr*)&epar->yo_sckaddr, &len_sckaddr );
    else  nap_delay( 1 );
...
int peek_read_udp( ENT_PARAM_S *epar, void *dbuf )
{
  ...
  if ( epar->nonblock_opt_flag ) { 
    ...
    if (peek_socket_ready(epar->socket,epar->tv_sec,epar->tv_usec) > 0) {
      statlen = recvfrom( epar->socket, (char*)dbuf, epar->in_pack_len, 0,
                      (struct sockaddr*)&epar->yo_sckaddr, &len_sckaddr );
      peek_counter = 0;
    }
    else {
  ...
  {
       case 0:
         /* fprintf(stderr,"peek_read_udp: RECV 0 len of data\n"); */
         break;
       case -1:
         ...
         break;
       default:  /* YES, INPUT DATA WAS READ */
         /** fprintf(stderr,"peek_read_udp: RECV %dlen of data\n", statlen); */
       break;
  }
...
int read_udp( ENT_PARAM_S *epar, void *dbuf )
{
  ...
  {
     case 0:
       /* fprintf(stderr,"read_udp: RECV 0 len of data\n"); */
       break;
     case -1:
       if ( errno != EWOULDBLOCK ) {
       ...
       break;
     default:  /* YES, INPUT DATA WAS READ */
       /** fprintf(stderr,"read_udp: RECV %dlen of data\n", statlen); */
     break;
  }                                         
...
int close_udp_socket( int psock )
{
  if (psock > 2) {
    shutdown(psock,2);
    close(psock);
...
int swap_byte( char *in, char *out, int blen, int wsize )
{
  register int  k;
  int  nword;
