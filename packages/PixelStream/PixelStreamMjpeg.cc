...
{
    if (this->port == that.port && this->host == that.host && this->path == that.path)
    {
        if (this->username == that.username && this->password == that.password) return true;
    }
    return false;
}
...
int PixelStreamMjpeg::readerInitialize(const std::string &hostspec, int portspec, const std::string &pathspec, const std::string &usrspec, const std::string &pwdspec)
{
#ifndef JPEG_ENABLED
