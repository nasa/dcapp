...
void _clean_xml_node(_ConfigContext *context, xmlNodePtr node, char *directory) {
    ...
    switch (elem_type) {
        ...
        case DC_APP_ELEM_TYPE_CONSTANT: {
            ...
            strncpy(cleaned_name, name, DC_VALUE_STRING_BUFFER_SIZE - 1);
            free(name);
...
void _dereference_constants(_ConfigContext *context, const char *in, char *out, size_t out_size) {
    ...
    for (int in_index = 0; in_index < in_length && out_index < out_size - 1; in_index++) {
        ...
        if (in[in_index] == '#' || in[in_index] == '$') {
            ...
            if (in[in_index] == '#') {
                const char *const_value = _get_const_by_name(context, subtext);
                if (const_value) {
...
static void _add_style(_ConfigContext *context, const char *name, DcAppElemType elem_type, xmlNodePtr xml_node) {
    if (name) {
        ...
        if (style_index == _STYLE_INDEX_UNDEFINED) {
            ...
            style_index = sbcount(context->sb_styles);
        }
...
static xmlChar *_get_style_attr(_ConfigContext *context, int style_index, DcAppElemType elem_type, const char *name) {
    xmlNodePtr style_xml_node = context->sb_styles[style_index].xml_nodes[elem_type];
    xmlChar   *value          = xmlGetProp(style_xml_node, (xmlChar *)(name));
    if (value) {
        return value;
    }
    return NULL;
...
// get xml attribute, taking styles/defaults into account
static char *_get_xml_node_attr(_ConfigContext *context, xmlNodePtr xml_node, DcAppElemType elem_type, int style_index, const char *name) {

    // default to whatever is in the xml node
    {
        char *value = (char *)xmlGetProp(xml_node, (xmlChar *)(name));
        if (value) {
            return value;
        }
    }

    // otherwise, fallback to the style
    if (style_index != _STYLE_INDEX_UNDEFINED) {
        xmlNodePtr style_xml_node = context->sb_styles[style_index].xml_nodes[elem_type];
        char      *value          = (char *)xmlGetProp(style_xml_node, (xmlChar *)(name));
        if (value) {
            return value;
        }
    }
    ...
    {
        xmlNodePtr default_xml_node = context->sb_styles[_STYLE_INDEX_DEFAULT].xml_nodes[elem_type];
        char      *value            = (char *)xmlGetProp(default_xml_node, (xmlChar *)(name));
        if (value) {
            return value;
        }
    }
...
static char *_get_xml_node_content(_ConfigContext *context, xmlNodePtr xml_node, DcAppElemType elem_type, int style_index) {
    ...
    // default to whatever is in the xml node
    {
        char *value = (char *)xmlNodeGetContent(xml_node);
        if (value) {
            return value;
        }
    }
    ...
    if (style_index != _STYLE_INDEX_UNDEFINED) {
        xmlNodePtr style_xml_node = context->sb_styles[style_index].xml_nodes[elem_type];
        char      *value          = (char *)xmlNodeGetContent(style_xml_node);
        if (value) {
            return value;
        }
    }
    ...
    {
        xmlNodePtr default_xml_node = context->sb_styles[_STYLE_INDEX_DEFAULT].xml_nodes[elem_type];
        char      *value            = (char *)xmlNodeGetContent(default_xml_node);
        if (value) {
            return value;
        }
    }
