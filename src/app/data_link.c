...
void dc_app_data_link_add_edge_tx(DcAppDataLinkContext *data_link, const char *command, DcAppVarIndex dcapp_var_index, const DcValue *initial_value) {
    if (!data_link || sbcount(data_link->sb_edges) == 0) return;
    DcAppEdgeConnection *connection = &data_link->sb_edges[sbcount(data_link->sb_edges) - 1];
    if (!connection->edge) return;
...
void dc_app_data_link_add_trick_tx(DcAppDataLinkContext *data_link, const char *path, const char *units, DcAppVarIndex dcapp_var_index, const DcValue *initial_value) {
    if (!data_link || sbcount(data_link->sb_tricks) == 0) return;
    DcAppTrickConnection *connection = &data_link->sb_tricks[sbcount(data_link->sb_tricks) - 1];
    if (!connection->trick) return;
...
void dc_app_data_link_update(DcAppDataLinkContext *data_link, DcAppLookup *lookup) {
    ...
    for (int ii = 0; ii < sbcount(data_link->sb_tricks); ii++) {
        ...
        if (trick_context->connected_var_index != DC_APP_VAR_INDEX_UNDEFINED) {
            DcAppValIndex value_index = dc_app_lookup_get_var_value_index(lookup, trick_context->connected_var_index);
        ...
        if (dc_trick_has_new_data(trick) && dc_trick_is_connected(trick)) {
            ...
            for (int jj = 0; jj < sbcount(trick_context->sb_rx_var_contexts); jj++) {
                ...
                dc_app_lookup_set_var_to_string(lookup, rx_var_context->dcapp_var_index, rx_buffer);
            }
        }
    }
    ...
    for (int ii = 0; ii < sbcount(data_link->sb_edges); ii++) {
        ...
        if (edge_context->connected_var_index != DC_APP_VAR_INDEX_UNDEFINED) {
            DcAppValIndex value_index = dc_app_lookup_get_var_value_index(lookup, edge_context->connected_var_index);
            DcValue      *value       = dc_app_lookup_get_value(lookup, value_index);
            if (value) {
