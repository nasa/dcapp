    ...
    void cleanXmlNode(xmlNodePtr node, std::string directory)
    {
        ...
        else
        {
            ...
            case DC_ELEM_TYPE_INCLUDE:
            {
                ...
                // check XML file
                int xmlResult = xmlCheckFilename(includeFilePath.c_str());
                if (xmlResult == 0)
                {
                    throw std::runtime_error("Failed to stat include file: " + includeFilePath);
                }
                else if (xmlResult == 2)
                {
                    throw std::runtime_error("Configuration file is a directory: " + includeFilePath);
                }
