...
int tdRegisterXFont(char *, int, char *);
void tdSetActiveWindow(tdWindow);
void tdSetFont(char *, float);
...
tdWindow tdOpenWindow(char *title, float xpos, float ypos, float width, float height, int align)
{
    XGCValues values;
...
tdWindow tdOpenFullScreen(char *title)
{
    WmHints hints;
...
tdImage *tdLoadImage(char *filespec, tdSize *size)
{
    ...
    image->xcolor = calloc(image->ncolors, sizeof(unsigned long));
...
tdGLContext *tdGLCreateContext(tdWindow winid)
{
    ...
    {
        if (curGL->parent->win == winid) break;
    }
...
int tdRegisterXFont(char *face, int size, char *spec)
{
    ...
    font[index].face = malloc(strlen(face)+1);
...
void tdSetFont(char *fontface, float fontsize)
{
    int i;
