...
TDImage *tdLoadBmpImage(char *filespec)
{
    ...
    if (infile == 0)
    {
        ...
        char *fullpath = malloc(strlen(tddata) + strlen(filespec) + 2);
    ...
    if (image == 0) return 0;
    image->pixel = malloc(npixels * sizeof(unsigned));
    ...
    if (ncols)
    {
        ...
        bmpindex = malloc(ncols * sizeof(unsigned));
        if (bmpindex == 0) return 0;
        bcm = malloc(ncols * sizeof(BmpColorMap));
        if (bcm == 0) return 0;
        fread(bcm, sizeof(BmpColorMap), ncols, infile);

        ct = calloc(ncols, sizeof(TmpColorTable));
        if (ct == 0) return 0;
        for (i=0; i<ncols; i++)
        {
            foundmatch = 0;
        ...
        image->color = calloc(colcount, sizeof(TDColorSpec));
        ...
        pixmap = malloc(imagewidth * sizeof(unsigned char));
    ...
    {
        BmpPixel *bmpimg = malloc(imagewidth * sizeof(BmpPixel));
        if (bmpimg == 0) return 0;

        pad = imagewidth%4;

        ct = calloc(npixels, sizeof(TmpColorTable));
        if (ct == 0) return 0;
        for (i=0; i<imageheight; i++)
        {
